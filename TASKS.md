...
- [x] Krum 策略類別
  ...
  - [x] 撰寫測試：驗證策略初始化與配置
  - [x] 實作：KrumStrategy 類別
  - [x] 測試通過確認：配置參數檢查

- [x] 距離計算實作
  - [x] 撰寫測試：驗證不同模型間距離計算
  - [x] 實作：`compute_distances` 函數
  - [x] 測試通過確認：使用已知向量對
...
- [x] 端到端防禦測試
  ...
  - [x] 撰寫測試：模擬真實訓練過程
  - [x] 實作：orchestrator 模組整合所有元件
  - [x] 測試通過確認：使用 Foundry forge 腳本模擬完整流程

- [x] 防禦效果評估
  - [x] 撰寫測試：比較有無防禦下的模型質量
  - [x] 實作：`defense_evaluation` 函數
  - [x] 測試通過確認：指標比較
